package com.demo.main.caching.config;
...
public class RedisCacheConfig implements CachingConfigurer {
	...
	List<CacheConfig> cacheConfigList = appConfig.getCacheConfig();
		...
		cacheConfigMap.forEach((k, v) -> {
			...
			redisConfigMap.put(k, RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofSeconds(v)));
		});

		RedisCacheManager rcm = RedisCacheManager.builder(jedisConnectionFactory()).cacheDefaults(cacheConfiguration()).transactionAware().build();
